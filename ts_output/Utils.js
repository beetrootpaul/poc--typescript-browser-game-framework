...
export class Utils {
    ...
    static measureTextSize(text) {
        const charSprites = PocTsBGFramework.drawApi.getFont()?.spritesFor(text) ?? [];
        return charSprites.reduce((sizeSoFar, nextSprite) => Xy.max(sizeSoFar, nextSprite.positionInText.add(nextSprite.sprite.size())), Xy.zero);
    }
